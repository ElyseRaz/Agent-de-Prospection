...
function KanbanColumnDropZone({
  ...
  return (
    <div
      ...
      className={cx(
        "flex w-72 shrink-0 flex-col gap-2 rounded-lg bg-secondary p-2.5 ring-1 ring-transparent transition-colors",
        isOver && "bg-brand-primary ring-brand",
      )}
    >
      <div className="flex items-center justify-between px-1">
        <p className="text-sm font-semibold text-secondary">{label}</p>
        <span className="text-xs text-tertiary">{prospects.length}</span>
      </div>
      <div className="flex min-h-16 flex-col gap-2">
        {prospects.length === 0 ? (
          <p className="rounded-md border border-dashed border-secondary p-3 text-center text-xs text-tertiary">
            Aucun prospect
          </p>
        ) : (
